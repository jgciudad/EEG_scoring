function collection = aggregate_sleeptransformer(nchan)
    ...
    for fold = 1 : Nfold
        ...
        sz = [11 11];
        varTypes = ["char","double","double","double","double","double","double","double","double","double","double"];
        varNames = ["filename","acc","prec_N","sens_N","f1_N","prec_R","sens_R","f1_R","prec_W","sens_W","f1_W"];
        ...
        for i = 1 : numel(test_s)
            ...
            t_test_data(i,:) = {sname,acc,myselectivity_i(2),mysensitivity_i(2),fscore_i(2),myselectivity_i(3),mysensitivity_i(3),fscore_i(3),myselectivity_i(1),mysensitivity_i(1),fscore_i(1)};
            
            a=9;
            
        end
    writetable(t_test_data,"C:\Users\javig\Documents\Drive\DTU\MASTER THESIS\Code\EEG_scoring\SleepTransformer_mice\evaluation/t_test_data_ST.xlsx")
