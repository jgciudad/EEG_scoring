...
for ii = 1:2:numel(varargin)
    switch lower(varargin{ii})
        ...
        case 'targetsignals_type'
            targetSignals_type = varargin{ii+1};
        case 'targetsignals_index'
            targetSignals_index = varargin{ii+1};
        otherwise
...
if isempty(targetSignals_index)
    targetSignals = 1:numel(hdr.label);
elseif iscell(targetSignals_index)||ischar(targetSignals)
    if strcmp(hdr.label{targetSignals_index{1}}(1:3), targetSignals_type{1})
        targetSignals = targetSignals_index{1};
    end
end
